...
%--------------------------------------------------------------------------
% InvModeBioLithRT computes he objective function for the optimization
% problem to retrieve the water component concentrations given observed
% Rrs, simulated Rrs, and the input below
function [Res] = InvModeBioLithRT_Copt(Fit)
...
for i = 1:length(lambda)
    a_W(i) = Absorp_W(lambda(i), wavelength_aw, absorpt_W); % abs. of pure water [1/m]
end
...
Edsa = E0_interp .* (Tr .^ 0.5) .* Taa .* (1 - Tas) .* Toz .* To .* Twv .* cos(sun_rad) .* Fa;
Eds = Edsr + Edsa;
...
% Total remote sensing reflectance above water surface
sigma = 0.03;
nW = 1.33;
rho_U = 0.54;
Q = 5;
